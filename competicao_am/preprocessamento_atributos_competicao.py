...
def gerar_atributos_ator(df_treino:pd.DataFrame, df_data_to_predict: pd.DataFrame) -> pd.DataFrame:
    ...
    # print("Ator", df_treino_boa.head())
    return df_treino_boa, df_data_to_predict_boa


def gerar_atributos_diretor(df_treino: pd.DataFrame, df_data_to_predict: pd.DataFrame) -> pd.DataFrame:
    obj_bag_of_actors = BagOfItems(min_occur=3)
    ...
    df_data_to_predict_boa = obj_bag_of_actors.aplica_bag_of_items(df_data_to_predict,
                                                                   ["dirigido_por"])
    # print("Diretor", df_treino_boa.head())
    return df_treino_boa, df_data_to_predict_boa


def gerar_atributos_escritores(df_treino: pd.DataFrame, df_data_to_predict: pd.DataFrame) -> pd.DataFrame:
    obj_bag_of_actors = BagOfItems(min_occur=3)
    df_treino_boa = obj_bag_of_actors.cria_bag_of_items(df_treino, ["escrito_por_1", "escrito_por_2"])
    df_data_to_predict_boa = obj_bag_of_actors.aplica_bag_of_items(df_data_to_predict,
                                                                   ["escrito_por_1", "escrito_por_2"])
    # print("Escritor", df_treino_boa.head())
    return df_treino_boa, df_data_to_predict_boa

def gerar_atributos_geral(df_treino:pd.DataFrame, df_data_to_predict: pd.DataFrame) -> pd.DataFrame:
    obj_bag_of_actors = BagOfItems(min_occur=3)
    df_treino_boa = obj_bag_of_actors.cria_bag_of_items(df_treino, ["ator_1", "ator_2", "ator_3", "ator_4", "ator_5", "dirigido_por", "escrito_por_1", "escrito_por_2"])
    df_data_to_predict_boa = obj_bag_of_actors.aplica_bag_of_items(df_data_to_predict,
                                                                   ["ator_1", "ator_2", "ator_3", "ator_4", "ator_5", "dirigido_por", "escrito_por_1", "escrito_por_2"])
    return df_treino_boa, df_data_to_predict_boa
